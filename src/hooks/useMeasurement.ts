...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'SCANNING_TIMEOUT':
      ...
      // Depth model is bundled — go straight to READY even without a detected plane.
      return {...state, phase: 'READY', scanningTimeoutId: null};
  ...
  useEffect(() => {
    const sub = AppState.addEventListener('change', (next: AppStateStatus) => {
      if (next === 'background') {
        ...
        // Re-enter READY after coming back from background.
        // Deliberately excluded 'inactive' — transient system UI (notification shade,
        // Modal window creation) triggers 'inactive' on Android and must not reset state.
        dispatch({type: 'START_SCANNING'});
      }
    });
    return () => {
