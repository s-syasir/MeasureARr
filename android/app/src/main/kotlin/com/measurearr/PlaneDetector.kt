...
class PlaneDetector {
    ...
    fun detect(
        ...
        try {
            ...
            if (confidence > 0.7 && !cameraMatrix.empty()) {
    ...
    private fun decomposeHomography(H: Mat, K: Mat): Pair<DoubleArray, Double> {
        ...
        for (i in rotations.indices) {
            val nx = normals[i].get(0, 0)?.firstOrNull() ?: continue
            val ny = normals[i].get(1, 0)?.firstOrNull() ?: continue
            val nz = normals[i].get(2, 0)?.firstOrNull() ?: continue
            if (nz <= 0) continue  // must face camera
